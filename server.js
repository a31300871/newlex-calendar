...
app.get('/api/listings', async (req, res) => {
  ...
  try {
    ...
    // If statewide search active (opt-in via ?statewide=1), filter to only listings in that state OR statewide_state matches
    const _statewideOptIn = req.query.statewide === '1' || req.query.statewide === 'true';
    if (_statewideOptIn && _zipState) {
      const rowsFiltered = rows.filter(r => {
        const itemState = zipToState(r.zipcode);
        return itemState === _zipState || r.statewide_state === _zipState;
      });
      rows = rowsFiltered;
    }
    if (!isAdmin) {
